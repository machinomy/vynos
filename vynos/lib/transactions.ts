...
import { getVersionPrefix } from './helpers'
...
export function closeChannel (description: string): Transaction {
  return {
    ...
    icon: getVersionPrefix() + '/frame/styles/images/close_channel-approved.png',
    time: Date.now(),
    amount: 0,
    fee: 0,
...
export function openChannel (title: string, description: string, from: string, to: string, amount: number): Transaction {
  return {
    ...
    title: title,
    description: description,
    icon: getVersionPrefix() + '/frame/styles/images/open_channel-approved.png',
    time: Date.now(),
    amount: amount,
    fee: 0,
